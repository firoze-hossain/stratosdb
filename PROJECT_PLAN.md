...
| Aggregation | `GROUP BY`/`HAVING`/`COUNT`/`SUM`/`AVG`/`MIN`/`MAX` done; CTEs done; no window functions | Full | Moderate |
| Subqueries | Scalar, `IN`/`NOT IN`, `EXISTS`/`NOT EXISTS` (correlated), and CTEs (single, non-recursive) done; no recursive queries | Full (scalar, correlated, `EXISTS`, CTEs, recursive) | Moderate |
...
| CTEs, `WITH RECURSIVE` | ✅ done (single, non-recursive `WITH`) | none | Reuses views' own `executeSelectOverView` path almost entirely - a CTE is "a view scoped to one statement" rather than a different mechanism, inheriting views' existing JOIN/aggregate rejection for free. A real concurrency risk was identified and designed around before it became a bug: storing the active CTE in shared state (like `views`) would race under concurrent connections using the same CTE name, so it's stored in `SessionState` (already per-connection) instead - verified directly with concurrent threads using an identical CTE name backed by different data. `WITH RECURSIVE` remains a genuinely different execution model (iterate to a fixpoint), not attempted here. |

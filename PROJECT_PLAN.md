...
Seven fronts now have real progress, all with measured/demonstrated proof rather than just claims (see PROGRESS.md for all of it): **Phase A's storage hardening** (B+Tree delete, vacuum, savepoints, and - discovered mid-round rather than planned - three independent restart-correctness bugs found and fixed: schema definitions never persisted at all, WAL redo duplicated data on every restart, and transaction commit status didn't survive a restart either), **Phase B's query engine depth** (`GROUP BY`/aggregates, hash join, a genuinely cost-based scan planner, and a real WHERE-clause expression tree with subqueries - which along the way surfaced and fixed a real bug: `AND`/`OR`/`NOT`/`LIKE`/`IN` were accepted by the grammar but silently misevaluated), **Phase C's hash index**, **Phase D's multi-statement transactions**, and **Phase E's views/autovacuum/slow-query logging**. All three restart-correctness bugs were found the same way: by actually testing a real restart through the SQL layer, which no earlier round had done, rather than by inspection.

What's left in Phase A: WAL-logging savepoints' own undo actions (now genuinely unblocked - the persisted commit-status log this round added removes what would have made "does a savepoint rollback survive a crash" unreliable to verify) and a free-space map (efficiency, not correctness). What's left in Phase B: merge join, window functions, and CTEs. What's left in Phase C: index-only scans (blocked on a visibility map, which doesn't exist yet), bitmap index, GiST/GIN-equivalents. What's left in Phase D: PostgreSQL wire protocol compatibility (the single highest-leverage ecosystem item on the whole list), SCRAM auth, client-side TLS certificate verification, streaming replication, connection pooling. What's left in Phase E: metrics, triggers, stored procedures, and real fine-grained concurrency control (page or table latching), which autovacuum's design deliberately worked around rather than solved.

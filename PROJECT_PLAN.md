...
What's left in Phase A: a free-space map (inserts still scan every existing page for room - an efficiency gap, not a correctness one) and savepoints (now unblocked - nested rollback points only make sense once transactions aren't purely auto-commit). What's left in Phase B: merge join, window functions, and CTEs. What's left in Phase C: index-only scans (blocked on a visibility map, which doesn't exist yet), bitmap index, GiST/GIN-equivalents. What's left in Phase D: PostgreSQL wire protocol compatibility (the single highest-leverage ecosystem item on the whole list, now unblocked), SCRAM auth, client-side TLS certificate verification, streaming replication, connection pooling. What's left in Phase E: metrics, triggers, stored procedures - and, worth naming honestly, real fine-grained concurrency control (page or table latching) for the storage engine generally, which autovacuum's design deliberately worked around rather than solved (see PROGRESS.md).

...
| Indexing | B+Tree with real delete (borrow/merge/root-collapse); real hash index (static, overflow chaining) for equality workloads; BRIN (block-range summaries), bitmap (per-value BitSets), and GIN (whole-word inverted text search) all done and actually consulted during scans, not just built; no index-accelerated joins | B-Tree, Hash, GiST, GIN, BRIN, SP-GiST | Moderate |
...
| Wire protocol | Real PostgreSQL wire protocol v3 (startup handshake + simple query protocol), verified against actual `psql` and an independent driver (`psycopg2`) | The format every pg client/tool already speaks | Moderate — simple query protocol done; extended protocol (prepared statements), SCRAM auth, and `pg_catalog` emulation (for `\dt`/tab-completion) remain |
...
| GIN | ✅ done, deliberately scoped | none | Real Postgres's GIN is a genuinely general framework (arrays, JSONB, full-text, via pluggable operator classes) - building that generality with no array/JSONB type in StratosDB to plug into it would produce a framework with nothing real to index. Implemented instead as GIN's single most common real application: a genuine inverted index for whole-word text search, powering a new `CONTAINS` operator that also works correctly with no index at all (same relationship `LIKE` has with indexes). Known gaps: whole-word matching only, no stemming/stop-words/ranking. |
| GiST | ⚪ not attempted | A range/interval or geometric data type | Real GiST is a framework for indexing arbitrary "consistent" predicates (classically: geometric overlap, range containment) via a balanced tree of predicates that generalize over their subtrees - not just another way to compare a single scalar value, which is all B+Tree/hash/BRIN/bitmap/GIN above already cover well. Without a range or geometric TYPE in StratosDB for such a predicate to mean anything, a "GiST" here would only ever be able to do what B+Tree already does - a hollow relabeling, not a real capability. The honest, narrowly-scoped version worth building (an interval-overlap index over a `(start, end)` column pair, GiST's classic real use case) needs multi-column index support first, which doesn't exist yet either - a real, named prerequisite, not a rejection of the feature itself. |
...
What's left in Phase A: WAL-logging savepoints' own undo actions (now genuinely unblocked) and a free-space map (efficiency, not correctness). What's left in Phase B: nothing on this project's own original "missing from Postgres" query-engine list remains - merge join, window functions, and recursive CTEs are all done; join reordering and true cost-based join planning (beyond the current row-count threshold) remain further work. What's left in Phase C: GiST (blocked on a range/interval or geometric type worth indexing, and on multi-column index support - see PROJECT_PLAN.md's own row above for the full reasoning), and range-scan index-only support (needs `BTreeIndex.rangeScan()` to return key-identifier pairs, not just identifiers); BRIN, bitmap, GIN, the visibility map, and equality-query index-only scans are now all done. What's left in Phase D: the extended query protocol (real server-side prepared statements), SCRAM auth, `\d TABLENAME`/`\l`/tab-completion (confirmed materially harder than `\dt`, which is now done - see above), client-side TLS certificate verification, streaming replication, connection pooling. What's left in Phase E: a proper `pg_stat`-shaped SQL surface (per-table/per-connection breakdowns, not just the flat snapshot `SHOW STATS` gives now), triggers, stored procedures, and real fine-grained concurrency control (page or table latching), which autovacuum's design deliberately worked around rather than solved.

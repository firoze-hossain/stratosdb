...
What's left in Phase A: WAL-logging savepoints' own undo actions (now genuinely unblocked) and a free-space map (efficiency, not correctness). What's left in Phase B: nothing on this project's own original "missing from Postgres" query-engine list remains - merge join, window functions, and recursive CTEs are all done; join reordering and true cost-based join planning (beyond the current row-count threshold) remain further work. What's left in Phase C: index-only scans (blocked on a visibility map, which doesn't exist yet), bitmap index, GiST/GIN-equivalents. What's left in Phase D: the extended query protocol (real server-side prepared statements), SCRAM auth, `\d TABLENAME`/`\l`/tab-completion (confirmed materially harder than `\dt`, which is now done - see above), client-side TLS certificate verification, streaming replication, connection pooling. What's left in Phase E: a proper `pg_stat`-shaped SQL surface (per-table/per-connection breakdowns, not just the flat snapshot `SHOW STATS` gives now), triggers, stored procedures, and real fine-grained concurrency control (page or table latching), which autovacuum's design deliberately worked around rather than solved.

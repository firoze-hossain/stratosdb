...
- **`mvn test` fails with `IOException: Failed to delete temp directory` / "The process cannot access the file because it is being used by another process" (Windows)**: this was a real bug, found and fixed via an actual Windows `mvn test` run - thank you for that, since my own build environment is Linux and never surfaced it. The tests never explicitly closed their `DiskManager`/`WALManager`/`BufferPoolManager` instances; Linux allows deleting a file that's still open (the handle just keeps the space reserved until released), Windows does not. Every test file now tracks and closes every such resource in `@AfterEach`, strictly after each test's own assertions have run - the underlying logic wasn't wrong, only the test cleanup was. If you pulled before this fix landed, `git pull` and retry.
